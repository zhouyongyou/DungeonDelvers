// scripts/set-baseuri-updated.ts
// 更新版本的 BaseURI 設定腳本 - 支援 API 端點和 IPFS 切換
...
async function main() {
  ...
  console.log(`👤 部署者錢包: ${deployer.address}`);
  ...
  for (const { name, address, type } of contractUpdates) {
    if (address && address !== '0x0000000000000000000000000000000000000000') {
      await updateContractBaseURI(name, address, type, baseURIType);
    } else {
