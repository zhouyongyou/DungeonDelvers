...
interface ConsoleStatement {
  ...
  line: number;
  originalText: string;
  type: 'log' | 'warn' | 'error' | 'debug' | 'info' | 'trace';
  context: string;
...
class ConsoleLogManager {
  ...
  private async analyzeFile(filePath: string): Promise<void> {
    ...
    lines.forEach((line, index) => {
      ...
      if (consoleMatch) {
        const type = consoleMatch[1] as ConsoleStatement['type'];
  ...
  private isInCatchBlock(lines: string[], lineIndex: number): boolean {
  ...
  private isDevelopmentOnly(lines: string[], lineIndex: number): boolean {
  ...
  private generateReplacement(line: string, type: ConsoleStatement['type']): string {
  ...
  async process(dryRun: boolean = true): Promise<void> {
    ...
    const fileGroups = new Map<string, ConsoleStatement[]>();
...
async function main() {
  ...
  if (command === 'analyze' || !command) {
    ...
    console.log('Usage:');
    console.log('  npm run manage-console-logs          # Analyze only');
    console.log('  npm run manage-console-logs dry-run  # Show what would be changed');
    console.log('  npm run manage-console-logs apply    # Apply changes');
  }
}
