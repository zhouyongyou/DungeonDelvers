// AltarOfAscensionV2Fixed.sol - 優化版本
...
contract AltarOfAscensionV2Fixed is Ownable, ReentrancyGuard, Pausable {
    ...
    event UpgradeAttempted(
        ...
        uint8 outcome,
        uint256 fee,
        uint8 vipLevel,           // VIP等級
        uint8 totalVipBonus      // 總VIP加成
    );
    ...
    constructor(address _initialOwner) Ownable(_initialOwner) {
        ...
        upgradeRules[1] = UpgradeRule({
            ...
            greatSuccessChance: 8,      // 原5% -> 8% (中間值)
            successChance: 77,          // 原65% -> 77% (總85%)
            partialFailChance: 13,      // 原28% -> 13%
            cooldownTime: 10 seconds,   // 修改為10秒
            isActive: true
        });
        ...
        upgradeRules[2] = UpgradeRule({
            ...
            greatSuccessChance: 6,      // 原4% -> 6% (中間值)
            successChance: 69,          // 原51% -> 69% (總75%)
            partialFailChance: 20,      // 原35% -> 20%
            cooldownTime: 10 seconds,   // 修改為10秒
            isActive: true
        });
        ...
        upgradeRules[3] = UpgradeRule({
            ...
            greatSuccessChance: 4,      // 原3% -> 4% (中間值)
            successChance: 41,          // 原32% -> 41% (總45%)
            partialFailChance: 40,      // 原45% -> 40%
            cooldownTime: 10 seconds,   // 修改為10秒
            isActive: true
        });
        ...
        upgradeRules[4] = UpgradeRule({
            ...
            greatSuccessChance: 3,      // 原2% -> 3% (中間值)
            successChance: 22,          // 原18% -> 22% (總25%)
            partialFailChance: 50,      // 維持50%
            cooldownTime: 10 seconds,   // 修改為10秒
            isActive: true
        });
    }
    ...
    function getPlayerVipInfo(address _player) external view returns (
        ...
        uint8 totalVipBonus,
        uint8 effectiveTotalBonus  // ★ 新增：應用上限後的實際加成
    ) {
        // 獲取當前VIP等級
        try IVIPStaking(dungeonCore.vipStakingAddress()).getVipLevel(_player) returns (uint8 level) {
    ...
    function batchSetAdditionalVIPBonus(
        ...
        require(_players.length <= 100, "Altar: Too many players in batch"); // 防止gas溢出
    ...
    function previewUpgradeSuccess(address _player, uint8 _baseRarity) external view returns (
        ...
        uint8 totalVipBonus,
        uint8 cappedVipBonus  // ★ 新增：應用上限後的VIP加成
    ) {
        UpgradeRule memory rule = upgradeRules[_baseRarity];
