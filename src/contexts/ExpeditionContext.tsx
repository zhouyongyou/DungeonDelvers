...
interface ExpeditionResult { success: boolean; reward: bigint; }
interface ExpeditionContextValue { showExpeditionResult: (result: ExpeditionResult) => void; }
            ...
            {result && (
                <Modal isOpen={!!result} onClose={handleClose} onConfirm={handleClose} title={result.success ? "遠征成功！" : "遠征失敗"} confirmText={result.success ? "太棒了！" : "返回"} isConfirming={false}>
                    <div className="text-center">
                        <img src={result.success ? `https://placehold.co/150x150/fde047/a16207?text=🏆` : `https://placehold.co/150x150/9ca3af/4b5563?text=⚔️`} alt={result.success ? "勝利" : "失敗"} className="mx-auto mb-4 rounded-full border-4 border-yellow-600/50" />
