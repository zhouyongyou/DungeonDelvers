...
import { AlertCircle, Coins, MessageCircle, Server } from 'lucide-react';
...
export const GameInfoSection: React.FC<GameInfoSectionProps> = ({ variant = 'full' }) => {
  const [copied, setCopied] = useState(false);
  
  // 獲取 SOULSHARD 地址
  const soulShardAddress = getContractWithABI('SOULSHARD')?.address;
  
  const handleCopy = () => {
    if (!soulShardAddress) return;
    navigator.clipboard.writeText(soulShardAddress);
    setCopied(true);
    ...
    return (
      <div className="bg-[#2D2640] rounded-2xl p-6 md:p-8 lg:p-10">
          ...
          {soulShardAddress && (
            <div className="flex items-center justify-center gap-2">
              ...
              <button 
                ...
                title="點擊複製地址"
              >
                <span className="text-sm group-hover:text-white transition-colors">{`${soulShardAddress.substring(0, 6)}...${soulShardAddress.substring(soulShardAddress.length - 4)}`}</span>
                {copied ? <span className="text-green-400 text-xs">已複製!</span> : <CopyIcon className="w-4 h-4 text-gray-400 group-hover:text-white transition-colors" />}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6 max-w-3xl mx-auto">
          {/* 重要提醒 - 突出顯示 */}
          <div className="bg-gradient-to-r from-yellow-900/50 to-orange-900/50 rounded-xl p-6 border border-yellow-500/50 backdrop-blur-sm">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <ul className="space-y-2 text-xs text-gray-200">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-yellow-400 mt-1">•</span>
                    <span>請關注我們的 <a href="https://t.me/Soulbound_Saga" target="_blank" rel="noreferrer" className="text-blue-400 underline hover:text-blue-300">Telegram</a> 獲取最新消息</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-yellow-400 mt-1">•</span>
                    <span>代幣發行後約一小時內會完成所有相關設置</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-yellow-400 mt-1">•</span>
          ...
          <div className="flex justify-center items-center gap-4 pt-4">
            ...
            <a href="https://t.me/Soulbound_Saga" target="_blank" rel="noreferrer" className="w-8 h-8 transition-transform hover:scale-110" aria-label="Telegram">
              <svg viewBox="0 0 240 240">
                <defs>
                  <linearGradient id="telegram-gradient-gameinfo" x1="120" y1="240" x2="120" gradientUnits="userSpaceOnUse">
                    <stop offset="0" stopColor="#1d93d2" />
                    <stop offset="1" stopColor="#38b0e3" />
                  </linearGradient>
                </defs>
                <circle cx="120" cy="120" r="120" fill="url(#telegram-gradient-gameinfo)" />
            ...
            <a href="https://pancakeswap.finance/" target="_blank" rel="noreferrer" className="flex items-center justify-center w-8 h-8 rounded-full transition-transform hover:scale-110" aria-label="PancakeSwap">
              <svg width="100%" height="100%" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="48" cy="48" r="48" fill="url(#paint0_linear_pancake)" />
                ...
                <defs>
                  <linearGradient id="paint0_linear_pancake" x1="48" y1="0" x2="48" y2="96" gradientUnits="userSpaceOnUse">
  ...
  return (
    <div className="space-y-4 px-4 md:px-0">
      {/* 核心遊戲代幣 */}
      <div className="bg-gradient-to-r from-yellow-900/30 to-orange-900/30 rounded-lg p-4 md:p-6 border border-yellow-500/30">
            ...
            {soulShardAddress && (
              <div className="flex items-center gap-2 mb-2">
                ...
                <button 
                  ...
                  title="點擊複製地址"
                >
                  <span className="group-hover:text-white transition-colors">{`${soulShardAddress.substring(0, 6)}...${soulShardAddress.substring(soulShardAddress.length - 4)}`}</span>
                  {copied ? <span className="text-green-400 text-xs">✓</span> : <CopyIcon className="w-3 h-3 text-gray-400 group-hover:text-white transition-colors" />}
                </button>
              </div>
            )}
      ...
      {/* 重要提醒 */}
      <div className="bg-gradient-to-r from-red-900/30 to-pink-900/30 rounded-lg p-4 md:p-6 border border-red-500/30">
        <div className="flex items-start gap-3">
      ...
      <div className="flex justify-center">
        <a 
          ...
          target="_blank" 
          rel="noreferrer"
          className="inline-flex items-center gap-2 bg-blue-600 text-white font-bold py-2 md:py-3 px-4 md:px-6 rounded-lg hover:bg-blue-700 transition-colors text-sm md:text-base"
        >
          <MessageCircle className="w-4 h-4 md:w-5 md:h-5" />
          <span>加入 Telegram 群組</span>
        </a>
      </div>
    </div>
  );
