...
export const Footer: React.FC = () => {
  ...
  // ★ 核心修正：加入型別防衛，確保 chain.id 是支援的型別
  const soulShardAddress = (chain && (chain.id === bsc.id || chain.id === bscTestnet.id)) 
    ...
    <footer className="bg-[#1F1D36] text-gray-300 mt-auto">
      <div className="bg-black/20 text-white text-xs backdrop-blur-sm">
        <div className="container mx-auto px-4 py-1 flex justify-between items-center text-xs">
          <span>
            網路: <span className={`font-bold ${chain?.id === bsc.id ? 'text-yellow-400' : 'text-green-400'}`}>
              {chain?.name ?? '未連接'}
            </span>
          </span>
      ...
      <div className="container mx-auto text-center py-6 px-4">
        ...
        <div className="flex justify-center items-center text-sm gap-2 mb-4">
          ...
          <button 
            ...
            disabled={isMainnet || isNavigating}
            className={`px-3 py-1 rounded-full text-xs transition-all duration-200 flex items-center gap-1.5 ${isMainnet ? 'bg-yellow-500 text-white font-bold shadow-md' : 'bg-gray-600 hover:bg-gray-500 text-gray-300'}`}
          >
            <span className={`block w-2 h-2 rounded-full ${isMainnet ? 'bg-green-300 animate-pulse' : 'bg-gray-400'}`}></span>
            正式服
          </button>
          <button 
            onClick={handleGoToTestnet} 
            disabled={isTestnet || isNavigating}
            className={`px-3 py-1 rounded-full text-xs transition-all duration-200 flex items-center gap-1.5 ${isTestnet ? 'bg-green-500 text-white font-bold shadow-md' : 'bg-gray-600 hover:bg-gray-500 text-gray-300'}`}
          >
            <span className={`block w-2 h-2 rounded-full ${isTestnet ? 'bg-green-300 animate-pulse' : 'bg-gray-400'}`}></span>
            體驗服
          </button>
