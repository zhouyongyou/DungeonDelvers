...
export const DungeonPage: React.FC = () => {
    ...
    const { data: dungeonsData, isLoading: isLoadingDungeons } = useReadContracts({
        contracts: Array.from({ length: 10 }, (_, i) => ({ ...dungeonCoreContract, functionName: 'dungeons', args: [BigInt(i + 1)] })),
    ...
    const handleDispatch = async (dungeonId: number) => {
        ...
        try {
            ...
            await writeContractAsync({ ...dungeonCoreContract, functionName: 'requestExpedition', args: [BigInt(selectedPartyId), BigInt(dungeonId)] });
        } catch (e: any) {} finally { setActionState(null); }
    };
        ...
        <section>
            ...
            <div className="mb-8 card-bg p-4 flex flex-col md:flex-row gap-4 items-center">
                <div className="w-full">
                    ...
                    <select value={selectedPartyId} onChange={e => setSelectedPartyId(e.target.value)} className="w-full p-2 border rounded-lg bg-white/80 h-10">
                        ...
                        {ownedParties?.map((p: AnyNft) => (<option key={(p as PartyNft).id.toString()} value={(p as PartyNft).id.toString()}>{p.name || `隊伍 #${p.id.toString()}`} (戰力: {(p as PartyNft).totalPower.toString()})</option>))}
                    </select>
                </div>
                <div className="w-full md:w-auto flex-shrink-0">
                    <h3 className="section-title invisible hidden md:block">操作</h3>
                    <ActionButton onClick={handleClaimRewards} disabled={!selectedPartyId || actionState?.isLoading} isLoading={actionState?.type === 'claim' && actionState.isLoading} className="w-full h-10 px-4">領取獎勵</ActionButton>
                </div>
            </div>
            {isLoadingDungeons ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)}</div> : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {dungeons.map(d => (<div key={d.id} className="card-bg p-4 flex flex-col"><h4 className="text-xl font-bold font-serif">{getDungeonName(d.id)}</h4><div className="flex-grow mt-2 text-sm"><p>要求戰力: {d.requiredPower.toString()}</p><p>基礎獎勵: ~$ {parseFloat(formatEther(d.rewardAmountUSD)).toFixed(2)}</p><p>成功率: {d.baseSuccessRate}%</p></div><ActionButton onClick={() => handleDispatch(d.id)} isLoading={actionState?.type === 'dispatch' && actionState.id === d.id.toString() && actionState.isLoading} className="w-full mt-4 h-10 py-2 rounded-lg">派遣遠征</ActionButton></div>))}
                </div>
            )}
        </section>
    );
