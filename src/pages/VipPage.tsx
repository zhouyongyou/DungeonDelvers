...
// 倒數計時器元件
const CountdownTimer: React.FC<{ unlockTime: number }> = ({ unlockTime }) => {
  const [timeLeft, setTimeLeft] = useState('');
  const queryClient = useQueryClient();

  useEffect(() => {
    const updateTimer = () => {
      const remaining = Math.max(0, unlockTime - Date.now() / 1000);
      if (remaining === 0) {
        setTimeLeft('可以解鎖');
        // 可以考慮在此處觸發一次 refetch
...
const VipPage: React.FC = () => {
    ...
    const { data: totalSupplyData } = useReadContract({ ...vipStakingContract, functionName: '_tokenIdCounter' });
    const { data: mintPrice } = useReadContract({ ...vipStakingContract, functionName: 'mintPriceUSD' });
    // 【修正】使用 BigInt(0) 替代 0n 來提高相容性
    const { data: soulShardPrice } = useReadContract({ ...getContract(chainId, 'dungeonCore'), functionName: 'getSoulShardAmountForUSD', args: [mintPrice || BigInt(0)], query: { enabled: !!mintPrice } });

    const { data: allowance, refetch: refetchAllowance } = useReadContract({ ...soulShardContract, functionName: 'allowance', args: [address!, vipStakingContract?.address!], query: { enabled: !!address && !!vipStakingContract }});
    
    const { data: stakedInfo, refetch: refetchStakedInfo } = useReadContract({ ...vipStakingContract, functionName: 'userStakes', args: [address!], query: { enabled: !!address }});
    ...
    const handleTransactionSuccess = () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['_tokenIdCounter'] });
    }
    ...
    const handleMint = async () => {
        if (needsApproval) {
            showToast("請先完成授權", "error");
            return;
        }
        try {
            const hash = await writeContractAsync({ ...vipStakingContract, functionName: 'mint'});
    ...
    const handleStake = async () => {
        if (!selectedUnstakedCardId) {
            showToast("請先選擇一張要質押的 VIP 卡", "error");
            return;
        }
        try {
            const hash = await writeContractAsync({ ...vipStakingContract, functionName: 'stake', args: [selectedUnstakedCardId]});
    ...
    const handleUnstake = async () => {
        try {
            const hash = await writeContractAsync({ ...vipStakingContract, functionName: 'unstake'});
    ...
    const unstakedVipCards = useMemo(() => {
        return nfts?.vipCards?.filter(card => card.id !== stakedInfo?.[0]);
    }, [nfts, stakedInfo]);

    const totalSupply = useMemo(() => {
        return totalSupplyData ? Number(totalSupplyData) - 1 : 0;
    }, [totalSupplyData]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="card-bg p-6 rounded-xl shadow-lg flex flex-col items-center justify-between h-full">
                    ...
                    <div className="text-center my-6">
                        ...
                        <p className="text-lg mt-2">價格: <span className="font-bold text-yellow-400">{soulShardPrice ? parseFloat(formatEther(soulShardPrice)).toFixed(2) : '...'}</span> $SoulShard</p>
                    </div>
                    {needsApproval ? (
                ...
                <div className="card-bg p-6 rounded-xl shadow-lg h-full">
                    ...
                    {stakedInfo && stakedInfo.tokenId > BigInt(0) ? (
                        <div className="text-center">
                           <p className="text-green-400 font-bold mb-2">已質押</p>
                           <p>卡片 ID: <span className="font-mono">#{stakedInfo.tokenId.toString()}</span></p>
                           <p>解鎖倒數: <CountdownTimer unlockTime={Number(stakedInfo.unlockTime)} /></p>
                           <ActionButton onClick={handleUnstake} isLoading={isPending} disabled={Date.now()/1000 < Number(stakedInfo.unlockTime)} className="w-full mt-4 h-10">取消質押</ActionButton>
                        </div>
                    ) : (
                        <div className="text-center">
