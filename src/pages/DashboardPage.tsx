...
// 將等級計算邏輯提取為純函式，方便重用和測試
const getLevelFromExp = (exp: bigint): number => {
    if (exp < 100n) return 1;
    // 使用 BigIntMath.sqrt 來處理大數開方，或轉換為 number (需注意精度)
...
const DashboardPage: React.FC<{ setActivePage: (page: Page) => void }> = ({ setActivePage }) => {
    ...
    const { data: vaultInfo, isLoading: isLoadingVault } = useReadContract({
        ...
        query: {
            ...
            select: (data: unknown) => {
                ...
                return { withdrawableBalance: data[0] as bigint, lastWithdrawTimestamp: data[1] as bigint, isFirstWithdraw: data[2] as boolean };
            },
        }
    });
    ...
    const currentTaxRate = useMemo(() => {
        ...
        const TAX_PERIOD = 24n * 60n * 60n; // 24 hours
        if (timeSinceLast >= TAX_PERIOD * 3n) return 0;
        ...
        const finalTax = MAX_TAX_RATE - taxReduction;
        return Number(finalTax > 0 ? finalTax : 0n);
    }, [vaultInfo]);
    ...
    const handleWithdraw = async () => {
        ...
        try {
            ...
            setTimeout(() => queryClient.invalidateQueries({ queryKey: ['playerInfo', address] }), 1000);
        } catch(e: any) { 
        ...
        <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="card-bg p-6 rounded-xl flex flex-col justify-center">
                    ...
                    <p className="text-xs text-red-400">當前稅率: {currentTaxRate}%</p>
                    <ActionButton onClick={handleWithdraw} isLoading={isWithdrawing} disabled={!vaultInfo || vaultInfo.withdrawableBalance === 0n} className="mt-2 h-10 w-full">
                        全部提領
                    </ActionButton>
                </div>
            </div>
