...
// 獲取合約地址（統一接口）
export function getContractAddress(contractName: string, chainId: number = 56): Address {
  // 標準化合約名稱
...
// 獲取所有合約地址
export async function getAllContractAddresses(): Promise<{ [key: string]: Address }> {
  const config = await configLoader.getConfig();
  const addresses: { [key: string]: Address } = {};
  
  Object.entries(config.contracts).forEach(([name, address]) => {
    addresses[name] = address as Address;
  });
