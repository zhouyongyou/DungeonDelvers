...
contract Hero is ERC721, Ownable, ReentrancyGuard, Pausable, IVRFCallback {
    ...
    function mintFromWallet(uint256 _quantity) external payable nonReentrant whenNotPaused {
        ...
        if (vrfManager != address(0)) {
            // VRF 總費用包含 vrfRequestPrice + vrfPlatformFee
            vrfFee = IVRFManager(vrfManager).getTotalFee();
            requiredPayment += vrfFee;
        }
        require(msg.value >= requiredPayment, "Hero: Insufficient payment for mint and VRF");
        // ===== VRF 改動結束 =====
    ...
    function mintFromVault(uint256 _quantity) external payable nonReentrant whenNotPaused {
        ...
        require(msg.value >= requiredPayment, "Hero: Insufficient payment for vault mint and VRF");
        // ===== VRF 改動結束 =====
    ...
    function onVRFFulfilled(uint256 requestId, uint256[] memory randomWords) external override {
        require(msg.sender == vrfManager, "Hero: Only VRF Manager can call");
        // 空實現：Hero 合約使用輪詢方式獲取 VRF 結果
        // 實際的揭示邏輯在 _revealWithVRF 中處理
    }
